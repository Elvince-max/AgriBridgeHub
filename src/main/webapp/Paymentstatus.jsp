...
<%-- ══════════════════════════════════════════════════════════
     CHECK CALLBACK MAP FIRST (before loading the bean)
     On every auto-refresh, we check if Safaricom's callback
     has arrived by reading from MpesaCallbackServlet.paymentResults.
     If the result is there, we update the session so the bean
     picks up the new status.
     ══════════════════════════════════════════════════════════ --%>
<%
    // Get the CheckoutRequestID stored in session when STK push was sent
    String txCode = (String) session.getAttribute("transactionCode");

    if (txCode != null) {
        // Check if Safaricom has called our callback with a result
        String callbackResult = com.agribridge.controller.MpesaCallbackServlet.paymentResults.get(txCode);

        if (callbackResult != null) {
            if (callbackResult.startsWith("Completed:")) {
                // Update session to Completed
                session.setAttribute("paymentStatus", "Completed");
                // Replace CheckoutRequestID with real M-Pesa receipt number
                String receipt = callbackResult.substring("Completed:".length());
                if (!receipt.isEmpty()) {
                    session.setAttribute("transactionCode", receipt);
                }
            } else {
...
<%-- ══════════════════════════════════════════════════════════
     Java Bean — loads Payment bean from SESSION scope.
     jsp:setProperty pulls updated values from session above.
     ══════════════════════════════════════════════════════════ --%>
...
<%-- Convenience variables — replaces Java boolean declarations --%>
...
<head>
    ...
    <%-- JSTL <c:choose> replaces Java ternary for page title --%>
    <title>
        <c:choose>
    ...
    <%-- <c:if> replaces Java if block for meta refresh --%>
    <c:if test="${isPending}">
...
<nav class="top-nav">
    <span class="brand-name">EgertonDairy</span>
    <c:choose>
...
<div class="card-box">
    ...
    <div class="order-item-row">
        ...
        <div class="item-price">
            KES <fmt:formatNumber value="${sessionScope.paymentAmount}" 
    ...
    <div class="totals-row">
        <span>Subtotal</span>
        <span>KES <fmt:formatNumber value="${sessionScope.paymentAmount}" 
            type="number" minFractionDigits="2"/></span>
    </div>
    <div class="totals-row">
        <span>Delivery Fee</span><span>KES 150.00</span>
    </div>
    ...
    <div class="totals-final">
        ...
        <span>KES <fmt:formatNumber 
            value="${sessionScope.paymentAmount + 150}" 
            type="number" minFractionDigits="2"/></span>
    </div>

    <%-- NOTE: Individual cart items will plug in here from Michael's cart module --%>
</div>
        ...
        <c:when test="${isPending}">
            <div class="full-status-card">
                ...
                <%-- EL reads transactionCode from Payment bean --%>
        ...
        <c:otherwise>
            <div class="full-status-card">
                ...
                <div style="display:flex; gap:12px; justify-content:center; margin-top:20px; flex-wrap:wrap;">
                    <%-- EL builds retry URL using orderId from session --%>
...
<!-- Footer -->
<footer>
    <span>EgertonDairy &copy; 2025 Egerton University.</span>
    <div>
