...
<head>
    ...
    <style>
        ...
        .search-result-item:hover { background-color: #f5f5f0; }
        .search-result-img { width: 40px; height: 40px; object-fit: cover; border-radius: 8px; }
        .search-result-info { flex: 1; }
        .search-result-name { font-weight: 600; font-size: 0.9rem; }
        .search-result-price { font-size: 0.8rem; color: #1e4a2f; }
        .add-to-cart-search {
        ...
        .add-to-cart-search:hover { background-color: #643f00; }
    </style>
</head>
        ...
        <c:choose>
            <c:when test="${empty cart or empty cart.items}">
                <div class="text-center py-12 text-stone-500">
                    ...
                    <a href="products" class="inline-block mt-4 text-[#00450D] underline">Continue Shopping</a>
                </div>
            </c:when>
            <c:otherwise>
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        <thead class="border-b border-stone-200 text-stone-500 text-sm">
                            <tr><th class="pb-3">Product</th><th class="pb-3">Quantity</th><th class="pb-3">Price</th><th class="pb-3 text-right">Subtotal</th></tr></thead>
                        <tbody>
                                    ...
                                    <td class="py-4">
                                        <div class="flex items-center gap-3">
                                            <img src="${item.imageUrl != null ? item.imageUrl : 'https://via.placeholder.com/50x50?text=Product'}" 
                                                 alt="${item.name}" class="w-12 h-12 object-cover rounded-lg">
                                            <div><div class="font-medium">${item.name}</div><div class="text-xs text-stone-400">Pasture-raised, fresh from Egerton farms</div></div>
                                        </div>
                                    </td>
                                    <td class="py-4">
                ...
                <div class="flex justify-between items-center mt-6 pt-4 border-t border-stone-200">
                    <a href="products" class="text-[#00450D] font-medium flex items-center gap-1"><span class="material-symbols-outlined text-base">arrow_back</span> Continue Shopping</a>
                    <a href="CartServlet?action=clear" class="text-red-600 hover:text-red-700 text-sm flex items-center gap-1" onclick="return confirm('Empty your entire cart?');"><span class="material-symbols-outlined text-base">delete</span> Empty Cart</a>
                </div>
            </c:otherwise>
        </c:choose>
    </div>
    ...
    <div class="w-full lg:w-96">
        <div class="bg-white rounded-2xl shadow-sm p-6 sticky top-24">
            ...
            <div class="space-y-2 text-sm">
                <div class="flex justify-between"><span class="text-stone-600">Subtotal</span><span class="font-medium">KES <fmt:formatNumber value="${cart.total}" pattern="#,##0.00"/></span></div>
                <!--<div class="flex justify-between"><span class="text-stone-600">Delivery Fee</span><span class="font-medium">KES 150.00</span></div>-->
                <!--<div class="flex justify-between"><span class="text-stone-600">Estimated Tax (16% VAT)</span><span class="font-medium">KES </span></div>-->
                <div class="border-t border-stone-200 pt-3 mt-3">
                    <div class="flex justify-between text-lg font-bold text-[#00450D]"><span>Grand Total</span><span>KES <fmt:formatNumber value="${cart.total}" pattern="#,##0.00"/></span></div>
                    <p class="text-xs text-stone-400 mt-1">Inclusive of all taxes and fees</p>
                </div>
            </div>
            <a href="checkout.jsp" class="mt-6 block w-full bg-[#1e4a2f] hover:bg-[#0f331f] text-white text-center font-semibold py-3 rounded-full transition">Proceed to Checkout</a>
            <div class="mt-6 bg-[#fefaf5] rounded-xl p-4 border border-[#e6dfd3]">
                <div class="flex items-center gap-2 mb-2"><span class="material-symbols-outlined text-[#835400]">payments</span><span class="font-bold text-[#835400]">M-Pesa Direct Checkout</span></div>
                <p class="text-xs text-stone-600 mb-3">A payment prompt will be sent to your registered phone number after you place your order.</p>
                <div class="flex justify-around text-stone-500 text-xs"><span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">shield</span> Secure SSL</span><span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">lock</span> Encrypted</span><span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">check_circle</span> Trusted</span></div>
            </div>
        </div>
    </div>
</div>
...
<footer class="bg-[#00450D] mt-16 py-12 px-8 flex flex-col md:flex-row justify-between items-center gap-4">
    <div class="flex flex-col gap-2"><span class="text-white font-headline font-bold text-xl">EgertonAgriBridgeHub</span><span class="text-xs uppercase tracking-wide text-stone-400">© 2024 EgertonAgriBridgeHub. Modern Pastoral Excellence.</span></div>
    <div class="flex flex-wrap justify-center gap-8"><a href="#" class="text-xs uppercase tracking-wide text-stone-400 hover:text-[#FCAB28]">Privacy Policy</a><a href="#" class="text-xs uppercase tracking-wide text-stone-400 hover:text-[#FCAB28]">Terms of Service</a><a href="#" class="text-xs uppercase tracking-wide text-stone-400 hover:text-[#FCAB28]">Farmer Login</a><a href="#" class="text-xs uppercase tracking-wide text-stone-400 hover:text-[#FCAB28]">Sustainability Report</a></div>
</footer>
